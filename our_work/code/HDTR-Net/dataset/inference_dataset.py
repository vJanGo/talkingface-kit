...
class InferenceDataset():
    ...
    def __getitem__(self, index):
        ...
        mouth_mask_img = torch.from_numpy(mouth_mask_img).permute(2, 0, 1).float().cpu()
        mouth_contour_img = torch.from_numpy(mouth_contour_img).permute(2, 0, 1).float().cpu()
        referece_img = torch.from_numpy(referece_img).permute(2, 0, 1).float().cpu()

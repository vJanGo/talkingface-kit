...
def get_niqe_score(path):
    ...
    for full_frame_file,fin_out_file in zip(full_frame_files,fin_out_files):
        ...
        img1 = np.array(Image.open(img1_path).convert('LA'))[:, :, 0]
        img2 = np.array(Image.open(img2_path).convert('LA'))[:, :, 0]
        NIQE_sum_full_frame = NIQE_sum_full_frame + niqe(img1)
        NIQE_sum_fin_out = NIQE_sum_fin_out + niqe(img2)
        count = count + 1
        
    avg_score_full_frame = NIQE_sum_full_frame/count
    avg_socre_fin_out = NIQE_sum_fin_out/count
    
    return avg_score_full_frame,avg_socre_fin_out
...
if __name__ == '__main__':
    ...
    full_frame_niqe_score, fin_out_niqe_score = get_niqe_score(args.path)
